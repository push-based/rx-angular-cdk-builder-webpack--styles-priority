...
exports.indexHtmlTransform = void 0;
const jsdom = require("jsdom");
const operators_1 = require("rxjs/operators");
const { JSDOM } = jsdom;
const criticalSlotElement = (document) => {
...
const stylesheetSlotElement = (document) => {
    const elems = document.querySelectorAll('head');
    const defaultElem = elems[elems.length - 1];
    if (!defaultElem) {
...
const preloadSlotElement = (document) => {
    const elems = document.querySelectorAll('head');
    const defaultElem = elems[elems.length - 1];
    if (!defaultElem) {
...
const prefetchSlotElement = (document) => {
    const elems = document.querySelectorAll('head');
    const defaultElem = elems[elems.length - 1];
    if (!defaultElem) {
 ...
 */
function indexHtmlTransform(options$, indexHtmlContent) {
    return options$
        ...
        if (critical) {
            getLastSlotElement('critical', criticalSlotElement, document).prepend(htmlToElement(getStylesTags(critical, 'critical')));
        }
        if (stylesheet) {
            getLastSlotElement('stylesheet', stylesheetSlotElement, document).appendChild(htmlToElement(getStylesTags(stylesheet, 'stylesheet')));
        }
        if (preload) {
            getLastSlotElement('preload', preloadSlotElement, document).appendChild(htmlToElement(getStylesTags(preload, 'preload')));
        }
        if (prefetch) {
            getLastSlotElement('prefetch', prefetchSlotElement, document).appendChild(htmlToElement(getStylesTags(prefetch, 'prefetch')));
        }
...
exports.indexHtmlTransform = indexHtmlTransform;
function getLastSlotElement(slotName, defaultElement, document) {
    const elems = document.querySelectorAll(`[data-stylesSlots="${slotName}"]`);
...
function getStylesTags(url, slotName = 'prefetch') {
    ...
    function link(slotName, url) {
        const asAttribute = url.split('.').pop() === 'css' ? 'as="style"' : 'as="script"';
        return `<link rel="${slotName}" data-stylseSlot="${slotName}" href="${url}" ${slotName !== 'stylesheet' ? asAttribute : ''}/>`;
    }
    function style(slotName, url) {
...
//# sourceMappingURL=index-html-transform.js.map