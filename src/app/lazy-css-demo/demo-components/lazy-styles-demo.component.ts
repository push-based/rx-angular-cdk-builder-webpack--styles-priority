...
export class LazyStylesDemoComponent implements OnInit {
  @Input() slot: 'preload' | 'prefetch' = 'prefetch';
  ...
  ngOnInit(): void {
    const lazyStylesLinkEl = Object.assign(document.createElement('link'), {
      href: `lazy${this.strategy ? `-${this.strategy}` : ''}-markdown-styles.css`,
      rel: 'stylesheet'
    });
    console.log(lazyStylesLinkEl);
    if (this.position === 'append') {
      document.head.querySelectorAll('link[data-rxa-styles-slot="' + this.slot + '"]')[0].append(lazyStylesLinkEl);
    } else {
